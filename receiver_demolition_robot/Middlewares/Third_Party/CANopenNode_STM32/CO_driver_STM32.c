...
/*
 * TX mailbox ABORT callbacks.
 *
 * When a frame cannot be transmitted (e.g. the CAN bus is unplugged and no other
 * node ACKs), bxCAN aborts the mailbox and HAL calls these *Abort* callbacks instead
 * of the *Complete* ones. The stock CanOpenSTM32 driver only handled completion, so
 * the software TX queue would wedge (buffers stay "full" forever) and the node would
 * stop transmitting and never recover after the bus came back. Treating an abort like
 * a completion releases the buffer and lets the driver continue, so the link
 * self-heals on reconnect (combined with AutoBusOff hardware recovery).
 */
void

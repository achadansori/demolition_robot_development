...
/**
  ...
  * @note   Uses state machine for packet synchronization with checksum validation
  * @retval None
  */
void LoRa_Receiver_IRQHandler(void)
{
    ...
    switch (rx_state)
    {
        ...
        case RX_STATE_RECEIVING_DATA:
            ...
            {
                // All 8 bytes received, now wait for checksum
        ...
        case RX_STATE_WAIT_CHECKSUM:
            // Validate checksum
            {
                ...
                // else: Checksum invalid, discard packet silently
